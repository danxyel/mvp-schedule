...
| Elección cliente sesión vs paquete queda registrada por staff (2026-08-03) | Decisión del MVP de reservas recurrentes: como no existe checkout online real (Stripe/MercadoPago sigue como stub, Sprint 4) ni una pantalla self-service donde el cliente elija modalidad al momento de "comprar", la elección entre pago por sesión y pago por paquete la **registra el staff** en el modal de crear serie (`CrearSerieModal.jsx`). El cliente puede indicar su preferencia en `notas_cliente` de la solicitud (camino 2) o por otro canal externo, y el admin refleja esa elección en `modalidad_cobro`. El admin define previamente cuáles modalidades están habilitadas (`cobro_por_sesion_habilitado` / `cobro_por_paquete_habilitado`). Cuando haya checkout online real, esta decisión se revisará para darle la elección al cliente antes del pago. |
