...
| Pestaña "Pendientes" en PanelAdmin + filtro de listado sin fecha (2026-07-31) | Frontend del nuevo diseño de reservas con confirmación manual. `GET /admin/reservas` ahora omite el filtro de fecha cuando se pasa `estado` sin `fecha` (antes `fecha` default = hoy siempre); sin `estado` ni `fecha` conserva el default de hoy. Esto permite a la pestaña "Pendientes" listar reservas `pendiente` de todas las fechas (las pendientes suelen ser a futuro). Lista asesores vía `GET /admin/usuarios` filtrando `rol==='asesor' && activo` — mismo patrón de GestionUsuarios; **no existe** endpoint de asesores por servicio, y `GET /admin/usuarios` es admin-only, así que la pestaña es plenamente funcional para admin/superadmin (un asesor verá el listado pero sin opciones de asesor, consistente con la pestaña Usuarios existente). El 409 `franja_ocupada` se muestra inline bajo la fila sin tumbar la lista; éxito = banner verde + refetch. `FlujReserva` muestra el estado `pendiente` como éxito azul "Solicitud recibida" (no es `errorReserva`). Commit 1 = backend (router + openapi.json), commit 2 = frontend (schema.ts + FlujReserva + PanelAdmin + VALIDACION). |
