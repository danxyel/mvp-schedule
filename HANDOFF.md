...
| Precio de paquete vuelve a la serie, deja de capturarse por cliente (2026-08-04) | Corrección de diseño confirmada por Daniel: el precio por sesión ya es fijo (`servicio.precio`, definido una sola vez), pero el precio de **paquete** se estaba escribiendo a mano por cada cliente al inscribirlo (`InscripcionSerie.precio_paquete`) — nada garantizaba que todos pagaran lo mismo, y el admin repetía el número cada vez. `precio_paquete` vuelve a `SerieReserva` (existía ahí en el diseño v1 de un solo cliente, se había movido a la inscripción en la migración multicliente) como precio único, uniforme para toda la serie, capturado una sola vez al crear/configurar la serie — igual que el de sesión. Al inscribir a un cliente, el admin ya no escribe ningún precio, solo elige la modalidad. Ver `PROMPT_H_precio_paquete_por_serie.md`. |
